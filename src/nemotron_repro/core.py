...
def format_left_to_right_candidate_detail(cipher_word, decoded_pattern, candidate, mapping):
    ...
    reverse_mapping = build_reverse_mapping(mapping)
    parts = []
    for pos, (cipher_char, decoded_char, candidate_char) in enumerate(zip(cipher_word, decoded_pattern, candidate)):
        if decoded_char != '?':
            if decoded_char == candidate_char:
                parts.append(f'{pos}〖{candidate_char}〗〖{decoded_char}〗fixed match')
                continue
            return {'valid': False, 'detail': f'{pos}〖{decoded_char}〗〖{candidate_char}〗fixed mismatch', 'reason': 'fixed mismatch'}
        if candidate_char in reverse_mapping and reverse_mapping[candidate_char] != cipher_char:
            return {'valid': False, 'detail': f'{pos}〖{candidate_char}〗〖({cipher_char})〗 unmapped mismatch', 'reason': 'unmapped mismatch'}
        parts.append(f'{pos}〖{candidate_char}〗〖({cipher_char})〗 unmapped match')
    parts.append(f'{len(candidate)} all match')
    return {'valid': True, 'detail': ' , '.join(parts), 'reason': 'valid'}
...
def build_candidate_word_scan_highscore_style(cipher_word, decoded_pattern, mapping, valid_candidates, selected_word):
    ...
    displayed_words = collect_cipher_candidate_display_words(cipher_word=cipher_word, decoded_pattern=decoded_pattern, mapping=mapping, valid_candidates=valid_candidates, selected_word=selected_word)
    ...
    for candidate in displayed_words:
        ...
        if candidate_len != len(cipher_word):
            ...
            continue
        left_to_right_result = format_left_to_right_candidate_detail(cipher_word=cipher_word, decoded_pattern=decoded_pattern, candidate=candidate, mapping=mapping)
    ...
    try:
        ...
        for record in unknown_word_records:
            ...
            if '?' not in current_decoded_pattern:
                ...
                continue
            valid_candidates = get_consistent_vocab_candidates(cipher_word=cipher_word, decoded_pattern=current_decoded_pattern, vocab=cipher_selection_vocab, mapping=mapping)
            selected_word = choose_best_candidate(pattern=current_decoded_pattern, candidates=valid_candidates, sentence_words=None, word_idx=None)
            candidate_scan_log = build_candidate_word_scan_highscore_style(cipher_word=cipher_word, decoded_pattern=current_decoded_pattern, mapping=mapping, valid_candidates=valid_candidates, selected_word=selected_word)
            solution_parts.append(candidate_scan_log)
            if selected_word == current_decoded_pattern:
                decoded_words[word_idx] = selected_word
                continue
            selected_update_log, new_pairs = build_selected_word_update_highscore_style(cipher_word=cipher_word, selected_word=selected_word, mapping=mapping)
            solution_parts.append(selected_update_log)
            previous_mapping = dict(mapping)
            for c_char, p_char in new_pairs:
                mapping[c_char] = p_char
            solution_parts.append(build_redecode_after_mapping_update_log(query_text=query_text, query_words=query_words, mapping=mapping, previous_mapping=previous_mapping))
            solution_parts.append(build_unknown_letters_after_mapping_update_log(query_words=query_words, mapping=mapping))
            decoded_words[word_idx] = selected_word
